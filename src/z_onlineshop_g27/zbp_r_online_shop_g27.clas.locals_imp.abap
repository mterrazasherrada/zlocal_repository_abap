CLASS lhc_Online_Shop_G27 DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.
...
CLASS lhc_Online_Shop_G27 IMPLEMENTATION.
  ...
  with VALUE #( ( %cid = 'My%CID_1'
      ...
      WITH VALUE #( ( %cid_ref = 'My%ItemCID_1'
                      %target  = VALUE #( (
                                          ...
                                          textobjecttype = 'B01'
                                          language       = 'E'
                                          plainlongtext  = 'item text created from PAAS API G27 - Workshop prep.'
                                        ) (
                                          %cid           = 'My%CCT_2'
                                          textobjecttype = 'B02'
                                          language       = 'E'
                                          plainlongtext  = 'item2 text created from PAAS API G27'
                                        ) )
              )   )
      ...
      zbp_r_online_shop_g27=>cv_pr_mapped = ls_pr_mapped.
...
DATA:
      online_shops TYPE TABLE FOR UPDATE zr_online_shop_G27,
      online_shop  TYPE STRUCTURE FOR UPDATE zr_online_shop_G27.

    READ ENTITIES OF zr_online_shop_G27 IN LOCAL MODE
       ENTITY Online_Shop
        ALL FIELDS
    ...
    LOOP AT lt_online_shop_result INTO DATA(online_shop_read).
      ...
      ENDIF.
    ENDLOOP.
    MODIFY ENTITIES OF zr_online_shop_G27 IN LOCAL MODE
    ENTITY Online_Shop UPDATE SET FIELDS WITH online_shops
    MAPPED   DATA(ls_mapped_modify)
    FAILED   DATA(lt_failed_modify)
    REPORTED DATA(lt_reported_modify).

    DATA: lt_create_pr_imp TYPE TABLE FOR ACTION IMPORT  zr_online_shop_G27~create_pr,
...
CLASS lsc_ZR_ONLINE_SHOP_G27 DEFINITION INHERITING FROM cl_abap_behavior_saver.
  PROTECTED SECTION.
...
CLASS lsc_ZR_ONLINE_SHOP_G27 IMPLEMENTATION.

  METHOD save_modified.
  data: lt_online_shop_as type STANDARD TABLE OF zshop_as_g27,
        ls_online_shop_as type                   zshop_as_g27.
        if zbp_r_online_shop_g27=>cv_pr_mapped-purchaserequisition is not INITIAL.
        loop at zbp_r_online_shop_g27=>cv_pr_mapped-purchaserequisition ASSIGNING FIELD-SYMBOL(<fs_pr_mapped>).
