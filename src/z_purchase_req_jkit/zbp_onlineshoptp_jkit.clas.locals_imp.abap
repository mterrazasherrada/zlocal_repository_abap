...
CLASS lhc_onlineshop IMPLEMENTATION.
  ...
  METHOD createPruchaseRequisitionItem.
    DATA: purchase_requisitions      TYPE TABLE FOR CREATE I_PurchaserequisitionTP,
          ...
          update_lines               TYPE TABLE FOR UPDATE zr_onlineshoptp_Jkit\\OnlineShop,
          update_line                TYPE STRUCTURE FOR UPDATE zr_onlineshoptp_jkit\\OnlineShop,
...
*    delivery_date = cl_abap_context_info=>get_system_date(  ) + 14.
    ...
    READ ENTITIES OF zr_onlineshoptp_jkit IN LOCAL MODE
      ENTITY OnlineShop
        ALL FIELDS WITH
        CORRESPONDING #( keys )
...
*                   purchaserequisitionitemtext
    ...
    zbp_onlineshoptp_jkit=>mapped_purchase_requisition-purchaserequisition = mapped_create_pr-purchaserequisition.
    ...
    MODIFY ENTITIES OF zr_onlineshoptp_jkit IN LOCAL MODE
      ENTITY OnlineShop
        UPDATE
...
*                                     ELSE if_abap_behv=>fc-o-enabled   )
  ...
  METHOD setInitialOrderValues.
    ...
    SELECT MAX( order_id ) FROM zaonlineshopjkit INTO @DATA(max_order_id). "active table
    SELECT SINGLE FROM zdonlineshop_jki FIELDS MAX( orderid ) INTO @DATA(max_orderid_draft). "draft table
  ...
  METHOD calculateTotalPrice.
    DATA total_price TYPE zr_onlineshoptp_jkit-TotalPrice.
...
CLASS lsc_ZR_ONLINESHOPTP_jkit DEFINITION INHERITING FROM cl_abap_behavior_saver.
  PROTECTED SECTION.
...
CLASS lsc_ZR_ONLINESHOPTP_jkit IMPLEMENTATION.

  METHOD save_modified.
    DATA : lt_online_shop_as        TYPE STANDARD TABLE OF zaonlineshopjkit,
           ls_online_shop_as        TYPE                   zaonlineshopjkit,
           lt_online_shop_x_control TYPE STANDARD TABLE OF zaonlineshopjkit.
    IF create-onlineshop IS NOT INITIAL.
      lt_online_shop_as = CORRESPONDING #( create-onlineshop MAPPING FROM ENTITY ).
      INSERT zaonlineshopjkit FROM TABLE @lt_online_shop_as.
    ENDIF.
...
*                      lt_online_shop_as = CORRESPONDING #( create-onlineshop MAPPING FROM ENTITY ).
        ...
        MODIFY zaonlineshopjkit FROM TABLE @lt_online_shop_as.
*           ENDSELECT.
      ENDLOOP.
    ENDIF.
    IF zbp_onlineshoptp_jkit=>mapped_purchase_requisition IS NOT INITIAL AND update IS NOT INITIAL.
      LOOP AT zbp_onlineshoptp_jkit=>mapped_purchase_requisition-purchaserequisition ASSIGNING FIELD-SYMBOL(<fs_pr_mapped>).
...
*      update zaonlineshop_xxx FROM  @( VALUE #(  purchase_requisition = ls_pr_key-purchaserequisition  pr_creation_date =  creation_date order_id = ls_online_shop-OrderID  ) ).
        UPDATE zaonlineshopjkit SET purchase_requisition = @ls_pr_key-purchaserequisition, pr_creation_date = @creation_date WHERE order_uuid = @ls_online_shop-OrderUUID.
      ENDLOOP.
      IF ls_pr_key-purchaserequisition IS NOT INITIAL.


        RAISE ENTITY EVENT zr_onlineshoptp_jkit~sendPurchaseRequisitionDetails
          FROM VALUE #(
...
*              email  = prdet-Email
    ...
    LOOP AT delete-onlineshop INTO DATA(onlineshop_delete) WHERE OrderUUID IS NOT INITIAL.
      DELETE FROM zaonlineshopjkit WHERE order_uuid = @onlineshop_delete-OrderUUID.
      DELETE FROM zdonlineshop_jki WHERE orderuuid = @onlineshop_delete-OrderUUID.
    ENDLOOP.
  ENDMETHOD.
