CLASS lsc_zbp_r_online_shop_g65 DEFINITION INHERITING FROM cl_abap_behavior_saver.
  PROTECTED SECTION.
    METHODS save_modified REDEFINITION.
ENDCLASS.

CLASS lsc_zbp_r_online_shop_g65 IMPLEMENTATION.

  METHOD save_modified.
    DATA : lt_online_shop_as TYPE STANDARD TABLE OF zshop_as_g65,
           ls_online_shop_as TYPE                   zshop_as_g65.
    IF zbp_i_online_shop_g65=>cv_pr_mapped-purchaserequisition IS NOT INITIAL.
      LOOP AT zbp_i_online_shop_g65=>cv_pr_mapped-purchaserequisition ASSIGNING FIELD-SYMBOL(<fs_pr_mapped>).
    ...
    IF create-online_shop IS NOT INITIAL.
      ...
      INSERT zshop_as_g65 FROM TABLE @lt_online_shop_as.
    ENDIF.
  ENDMETHOD.
ENDCLASS.

CLASS lhc_zbp_r_online_shop_g65  DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.
...
CLASS lhc_zbp_r_online_shop_g65  IMPLEMENTATION.
...
TRY.
      ...
      zbp_i_online_shop_g65=>cv_pr_mapped = ls_pr_mapped.
...
DATA:
      online_shops TYPE TABLE FOR UPDATE zi_online_shop_G65,
      online_shop  TYPE STRUCTURE FOR UPDATE zi_online_shop_G65.

    READ ENTITIES OF zi_online_shop_g65 IN LOCAL MODE
       ENTITY Online_Shop
        ALL FIELDS
    ...
    LOOP AT lt_online_shop_result INTO DATA(online_shop_read).
      ...
      ENDIF.
    ENDLOOP.
    MODIFY ENTITIES OF zi_online_shop_g65 IN LOCAL MODE
    ENTITY Online_Shop UPDATE SET FIELDS WITH online_shops
    MAPPED   DATA(ls_mapped_modify)
    FAILED   DATA(lt_failed_modify)
    REPORTED DATA(lt_reported_modify).

    DATA: lt_create_pr_imp TYPE TABLE FOR ACTION IMPORT  zi_online_shop_g65~create_pr,
