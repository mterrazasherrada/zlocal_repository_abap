CLASS lsc_zbp_i_onlineshop_H18 DEFINITION INHERITING FROM cl_abap_behavior_saver.
  PROTECTED SECTION.
    METHODS save_modified REDEFINITION.
ENDCLASS.

CLASS lsc_zbp_i_onlineshop_H18 IMPLEMENTATION.

  METHOD save_modified.
    DATA : lt_online_shop_as TYPE STANDARD TABLE OF zshop_as_H18,
           ls_online_shop_as TYPE                   zshop_as_H18.
    IF zbp_i_onlineshop_H18=>cv_pr_mapped-purchaserequisition IS NOT INITIAL.
      LOOP AT zbp_i_onlineshop_H18=>cv_pr_mapped-purchaserequisition ASSIGNING FIELD-SYMBOL(<fs_pr_mapped>).
        CONVERT KEY OF i_purchaserequisitiontp FROM <fs_pr_mapped>-%pid TO DATA(ls_pr_key).
        <fs_pr_mapped>-purchaserequisition = ls_pr_key-purchaserequisition.
      ENDLOOP.
    ENDIF.

    IF create-online_shop_H18 IS NOT INITIAL.
      " Creates internal table with instance data
      lt_online_shop_as = CORRESPONDING #( create-online_shop_H18 ).
      lt_online_shop_as[ 1 ]-purchasereqn =  ls_pr_key-purchaserequisition .

      INSERT zshop_as_H18 FROM TABLE @lt_online_shop_as.
    ENDIF.
  ENDMETHOD.
ENDCLASS.

CLASS lhc_zbp_i_online_shop_H18  DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.

    METHODS get_instance_authorizations FOR INSTANCE AUTHORIZATION
      IMPORTING keys REQUEST requested_authorizations FOR  online_shop_H18 RESULT result.

    METHODS create_pr FOR MODIFY
      IMPORTING keys FOR ACTION online_shop_H18~create_pr.

    METHODS update_inforecord FOR MODIFY
      IMPORTING keys FOR ACTION online_shop_H18~update_inforecord.

    METHODS calculate_order_id FOR DETERMINE ON MODIFY
      IMPORTING keys FOR online_shop_H18~calculate_order_id.


ENDCLASS.

CLASS lhc_zbp_i_online_shop_H18  IMPLEMENTATION.
...
*  if a new package is ordered, trigger a new purchase requisition
      ...
      WITH VALUE #( ( %cid_ref = 'My%ItemCID_1'
                      %target  = VALUE #( (
                                          ...
                                          textobjecttype = 'B01'
                                          language       = 'E'
                                          plainlongtext  = 'item text created from PAAS API H18'
                                        ) (
                                          %cid = 'My%CCT_2'
                                          textobjecttype = 'B02'
                                          language       = 'E'
                                          plainlongtext  = 'item2 text created from PAAS API H18'
                                        ) )
              )   )
      ...
      zbp_i_onlineshop_H18=>cv_pr_mapped = ls_pr_mapped.
    ENDLOOP.
...
* Update an existing info record
  ...
  METHOD calculate_order_id.
    DATA:
      online_shops TYPE TABLE FOR UPDATE zi_onlineshop_H18,
      online_shop  TYPE STRUCTURE FOR UPDATE zi_onlineshop_H18.

    SELECT MAX( order_id ) FROM zonlineshop_H18 INTO @DATA(max_order_id).

    READ ENTITIES OF zi_onlineshop_H18 IN LOCAL MODE
       ENTITY Online_Shop_H18
        ALL FIELDS
    ...
    MODIFY ENTITIES OF zi_onlineshop_H18 IN LOCAL MODE
    ENTITY Online_Shop_H18 UPDATE SET FIELDS WITH online_shops
    MAPPED   DATA(ls_mapped_modify)
    FAILED   DATA(lt_failed_modify)
    REPORTED DATA(lt_reported_modify).

    DATA: lt_create_pr_imp TYPE TABLE FOR ACTION IMPORT  zi_onlineshop_H18~create_pr,
...
* if a new package is ordered, trigger a new purchase requisition
    IF lt_failed_modify IS INITIAL.
      MODIFY ENTITIES OF zi_onlineshop_H18 IN LOCAL MODE
      ENTITY Online_Shop_H18 EXECUTE create_pr FROM CORRESPONDING #( lt_create_pr_imp ) "CORRESPONDING #( keys )
